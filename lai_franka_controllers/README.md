...
**Terminal 2:** Start the HQP Sine Trajectory Generator

```bash
# Streams position and velocity sine-wave targets into the HQP solver
ros2 run lai_franka_controllers hqp_joint_trajectory_test_node

```

> **Note:** Unlike Cartesian mode, the Joint mode does *not* feature an automated point-to-point trajectory planner. The node above is strictly a diagnostic sine-wave generator used to validate HQP optimization bounds. To send specific static joint targets, you must manually publish to the `/hqp_reference_generator_node/target_joint` topic.

Here is the updated **Usage Guide** section for `lai_franka_controllers/README.md`.

I have updated **Scenario C** to detail both ways to publish Cartesian goals (via the smooth Quintic Trajectory Generator and directly to the HQP node), complete with exact `ros2 topic pub` commands.

### Scenario C: Cartesian Operations (Planner, Direct Targets, or Stress Test)

The standard usage for Cartesian operations. *(Ensure `primary_task.mode` is set to `"cartesian"` in your YAML).*

#### Option 1: Using the Smooth Trajectory Planner (Recommended)
...
* **Send a Goal:** Publish your target to `/goal_pose`. The planner intercepts this, generates a smooth quintic S-curve trajectory profile, and streams it incrementally into the HQP solver:
```bash
...
```bash
ros2 topic pub --once /goal_pose geometry_msgs/msg/PoseStamped \
"{header: {frame_id: 'fr3_link0'}, pose: {position: {x: 0.5, y: 0.1, z: -0.1}, orientation: {w: 1.0, x: 0.0, y: 0.0, z: 0.0}}}"

```

#### Option 2: Direct Target Ingestion (Bypassing the Planner)

If you don't need a smooth quintic planner and want to feed static 6D pose targets directly into the HQP reference generator, you can bypass `trajectory_generator_node` and publish straight to the reference generator's ingestion topic:
...
**Terminal 2:** Publish a direct target pose
...
* `/joint_pd_velocity_controller/joint_commands` *(Commands generated by the HQP)*
* `/joint_pd_velocity_controller/output_dq_cmd` *(Actual commands sent to the hardware driver)*
