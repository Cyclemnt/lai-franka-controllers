/// @file hqp_position_test_node.cpp
/// @brief Multi-priority task optimization loop execution details.

#include "lai_franka_controllers/hqp_position_test_node.hpp"
#include <chrono>
...
HqpPositionTestNode::HqpPositionTestNode() : Node("hqp_position_test_node") {
    ...
    joint_cmd_pub_        = this->create_publisher<std_msgs::msg::Float64MultiArray>("/hqp_joint_position_controller/commands", 10);
    ...
    target_pose_sub_      = this->create_subscription<geometry_msgs::msg::PoseStamped>(
        "~/target_pose", 10, std::bind(&HqpPositionTestNode::target_pose_callback, this, std::placeholders::_1));
...
void HqpPositionTestNode::joint_state_callback(const sensor_msgs::msg::JointState::SharedPtr msg) {
    ...
    if (matched_joints == joint_names_.size()) {
        ...
        RCLCPP_INFO(this->get_logger(), "Internal Reference Map Aligned. 100 Hz HQP Optimization Active.");
    }
}

void HqpPositionTestNode::target_pose_callback(const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
    std::lock_guard<std::mutex> lock(data_mutex_);
...
void HqpPositionTestNode::timer_callback() {
    if (!is_initialized_) return;
...
int main(int argc, char * argv[]) {
    ...
    rclcpp::spin(std::make_shared<lai_franka_controllers::HqpPositionTestNode>());
    rclcpp::shutdown();
    return 0;
}